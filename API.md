...
AI 에이전트가 부를 수 있게 만든다면, 도구를 하나로 묶지 마세요. "출발지와 도착지를
받아서 접수하는 도구" 하나만 두면 에이전트가 사용자 확인 없이 한 번에 차를 부릅니다.

최소 두 개로 나누고 사이에 사람을 넣으세요.

**주소 확인 도구** — 문자열을 받아 확정된 주소나 후보 목록을 돌려줍니다. 접수는 하지
않습니다. `ambiguous`면 `candidates`를 그대로 돌려주어 에이전트가 사용자에게 읽어주고
고르게 합니다. 도구가 임의로 하나를 골라 돌려주면 안 됩니다. 번지가 없으면 그 사실을
돌려주어 사용자가 더 구체적인 주소를 말하게 합니다.

**접수 도구** — 이미 확정된 주소만 받습니다. 사용자가 입력한 원본 문자열을 받지 마세요.
받으면 확인 단계를 건너뛸 길이 생깁니다.

도구 설명(description)에 조건을 적어두세요. 에이전트는 그걸 읽고 판단합니다.

```
출발지·도착지는 주소 확인 도구로 확정한 값만 넣는다.
사용자에게 출발지, 도착지, 시각을 확인받은 뒤에만 호출한다.
후보가 여러 개였다면 사용자가 직접 고른 것이어야 한다.
```

조회와 취소는 별개 도구로 두세요. 취소는 되돌릴 수 없고 복지콜 대기 규정이 걸립니다.

### 도구 승인 화면을 확인 단계로 쓰기

Claude Code처럼 도구 호출 전에 사용자 승인을 받는 호스트라면, 그 화면이 마지막 확인이
됩니다. 이때 인자를 사람이 읽을 수 있는 평문으로 넘기세요. 좌표나 내부 식별자만 넘기면
승인 화면에서 어디로 가는 예약인지 알 수 없습니다.

승인 화면이 없는 호스트도 있으므로, 그것만 믿지 말고 위의 도구 분리를 함께 하세요.
